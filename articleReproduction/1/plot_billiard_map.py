...
# scene / physics: must match integrator_sample_gpu.cpp and physics.h
WALL_R = 1.0
...
# colour identifies the particle. Index is an ordered quantity here (particle i
# launches with vx = 2i/(N-1)), so a light->dark ramp is the right encoding.
n_p = int(pid.max()) + 1
norm = Normalize(vmin=0, vmax=n_p - 1)
colors = CMAP(norm(pid))
...
ax.scatter(theta, alpha, s=13, c=colors, edgecolors=SURFACE,
...
ax.set_title("Collision map of the circular gravitational billiard",
             color=INK, fontsize=14, pad=30, loc="left")
ax.text(0.0, 1.008,
        f"{len(x)} collisions · {n_p} orbits · "
...
cb = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=CMAP), ax=ax,
                  ...
                  ticks=[0, 25, 50, 75, n_p - 1])
cb.set_label("particle index", color=INK_2, fontsize=10)
